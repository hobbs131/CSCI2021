...
valg_penalty_max=5                         # maximum point deduction for valgrind problems
...
major_sep
printf "#PROBLEM 3 tree_main.c TESTS\n"

# Run normal tests: capture output and check against expected
if [ "$run_norm" = "1" ]; then
    minor_sep
    printf "tree_main NORMAL TESTS\n"
...
if [ "$run_valg" = "1" ]; then
    minor_sep
    printf "tree_main.c VALGRIND TESTS\n"
