...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  // block does not have sufficient size for a split
  if (block -> size < new_size + EL_BLOCK_OVERHEAD){
    return NULL;
  }
  // changes size to block and adjusts footer
  size_t block_size = block -> size;
  block -> size = new_size;
  el_blockfoot_t *block_foot = el_get_footer(block);
  block_foot -> size = new_size;

  // Assigning block above to corect size and adds footer
  el_blockhead_t *split_block = el_block_above(block);
  split_block -> size = block_size - new_size - EL_BLOCK_OVERHEAD;
  el_blockfoot_t *split_block_foot = el_get_footer(split_block);
  split_block_foot -> size = block_size - new_size - EL_BLOCK_OVERHEAD;
...
void *el_malloc(size_t nbytes){
  ...
  el_blockhead_t *extra_space = el_split_block(block,nbytes);
  extra_space -> state = EL_AVAILABLE;
  block -> state = EL_USED;
  el_add_block_front(el_ctl.used, block);
  el_add_block_front(el_ctl.avail, extra_space);
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  lower -> size = lower -> size + above -> size + EL_BLOCK_OVERHEAD;
